                ...
                <form id="frm_add_event" name="frm_add_event" method="post" action="<?php echo $this->Html->url('/') ?>events/add" class="login-subscription" enctype="multipart/form-data">
                    ...
                    <ul>
                        <li>
    				        ...
    				        <input type="text" name="data[Event][title]" title="" value="<?php echo isset($data['title']) ? $data['title'] : ''; ?>" class="text" placeholder="Tên sự kiện">
                        </li>
                        <li>
    						<label>Địa điểm</label>
    						<input type="text" name="data[Event][address]" title="" value="<?php echo isset($data['address']) ? $data['address'] : ''; ?>" class="text" placeholder="Địa điểm">
    					</li>
    					<li class="w50">
    					...
    					<li class="pt-radio">
    						<label>Chi phí</label>
    						<input type="radio" name="is_name5" title="" value="" class="text">
    						<span>Miễn phí</span>
    						<input type="radio" name="is_name5" title="" value="" class="text">
    					...
    					<li>
    						...
    						<input type="text" name="is_name5" title="" value="" class="text" placeholder="Nhập số điện thoại của bạn">
    					</li>
    					<li>
    						<label>Hình ảnh</label>
    						<input type="file" id="fileUpload" name="fileUpload" class="validate[required]">
    					</li>
    					<li>
    						<label>Mô tả</label>
    						<textarea id="message" name="message" cols="50" rows="10" class="validate[required]"></textarea>
    					</li>
    					<li>
...
<div class="content-center">
    <div class="content-event">
        <?php if (isset($error)){?>
            <div class="error"><?php echo $error ?></div>
        <?php } ?>
        <h2 class="titel">ĐĂNG SỰ KIỆN</h2>
            ...
            <form id="frm_add_event" name="frm_add_event" method="post" action="<?php echo $this->Html->url('/') ?>events/add" class="login-subscription" enctype="multipart/form-data">
                <fieldset>
                    ...
                    <ul>
                        <li>
                            <label>Tên sự kiện *</label>
                            <input type="text" name="data[Event][title]" title="" value="<?php echo isset($data['title']) ? $data['title'] : ''; ?>" id="title" class="validate[required] text"/>
                        </li>
                        <li>
                            <label>Địa điểm *</label>
                        ...
                        <li>
                            <label>Lĩnh vực</label>
                            <div class="select-1">
                                <select id="careers" name="data[Event][career_id]">
                                    <?php foreach ($careers as $career) {?>
                                    <option value="<?php echo $career['Career']['id']; ?>"  <?php if ($career['Career']['id'] == $careerId) echo 'selected="selected"'; ?>>
                                        <?php echo $career['Career']['name']; ?>
                                    </option>
